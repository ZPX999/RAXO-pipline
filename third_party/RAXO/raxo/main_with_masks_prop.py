...
parser.add_argument('--dino_repo', required=True, help='Path to the local DINOv3 repository')
parser.add_argument('--dino_weights', required=True, help='Path to the DINOv3 checkpoint')
parser.add_argument('--dino_model', default='dinov3_vitb16', help='DINOv3 torch hub model name')
parser.add_argument('--dino_image_size', type=int, default=256, help='DINOv3 input image size')
parser.add_argument('--dino_patch_size', type=int, default=16, help='DINOv3 patch size')
...
def build_model(args):
    if args.dino_image_size % args.dino_patch_size != 0:
        raise ValueError("DINOv3 image size must be divisible by patch size")

    transform = T.Compose([
        T.Resize((args.dino_image_size,args.dino_image_size) ,interpolation=T.InterpolationMode.BICUBIC),
        T.ToTensor(),
    ...
    dinov3_vitb16 = dinov3_vitb16.cuda()
    dinov3_vitb16.eval()
    
    return transform, dinov3_vitb16


def assemble_prototypes(prototypes_path, vocab, args):
    ...
    for values in support_set.values():
        if (
            values.get('dino_model') != args.dino_model
            or values.get('dino_image_size') != args.dino_image_size
            or values.get('dino_patch_size') != args.dino_patch_size
        ):
            raise ValueError(
                "The prototypes were not built with the current DINOv3 configuration. "
                "Please rebuild the prototypes."
            )
...
def main_known(args):
    ...
    prototypes, cat_name_in_each_index, cat_in_each_index_original = assemble_prototypes(args.prototypes, vocab, args)
    ...
    # 3) Build dino model
    transforms, model = build_model(args)
    
    # 3) Build dataloader
    data = CocoPrototypes_with_masks(
        coco,
        args.image_path,
        transforms,
        DINO_PATH_SIZE=args.dino_patch_size,
    )
    ...
    for batch in tqdm(dataloader):
        ...
        if outputs.shape[1] != masks.shape[1]:
            raise ValueError(
                f"DINOv3 tokens ({outputs.shape[1]}) and masks ({masks.shape[1]}) do not match"
            )
        if outputs.shape[-1] != prototypes.shape[-1]:
            raise ValueError(
                f"DINOv3 features ({outputs.shape[-1]}) and prototypes "
                f"({prototypes.shape[-1]}) do not match"
            )
