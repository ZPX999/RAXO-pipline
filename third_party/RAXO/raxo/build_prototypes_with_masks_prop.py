...
def compute_full_image_negative_prototype(model, dataloader):
    sum_tensor = torch.zeros(model.embed_dim)
    n_elements = 0
...
def compute_prototypes(model, dataloader, mapper_real_to_name, vocab):
    ...
    for batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            For example: If the input image is of size 256x256 and the patch size is 16x16, the number of
            patches (tokens) per image = ( 256 / 16 ) x ( 256 / 16 ) = 16 x 16 = 256.
            768: This is the hidden dimension of the ViT. Each token is represented as a 768-dimensional embedding vector
            in the DINOv3-base model.
            """
...
def main(args):
    ...
    # Keep the original prototype dictionary structure and attach enough information
    # to prevent accidental reuse with another DINO feature space.
    for values in prototypes.values():
        values['dino_model'] = args.dino_model
        values['dino_image_size'] = args.dino_image_size
        values['dino_patch_size'] = args.dino_patch_size
